...
class SocketMT5ProgramData(PyClass):
    event_time: str
...
class SocketMt5Handler:
    @staticmethod
    async def emit_mt5_program_data(data: SocketMT5ProgramData) -> None:
        data_dict = data.to_dict()
        await sio.emit(SocketMessage.SOCKET_MT5_PROGRAM_DATA, data_dict)  # pyright: ignore[reportUnknownMemberType]
